...
<table class="table table-striped">
    ...
    <tbody>
    <?php
    $query = mysql_query("SELECT `messages`.`id`, `idUserFrom`, `users_from`.`login`, `theme`, `time`, `isRead` "
        ."FROM `messages` "
        ."LEFT JOIN `users` as `users_from` ON `users_from`.`id` = `messages`.`idUserFrom` "
        ."LEFT JOIN `users` as `current_user` ON `current_user`.`id` = `messages`.`idUserTo` "
    ...
    while ($result = mysql_fetch_assoc($query)) {
        ...
        echo "<tr".$tr_style."><td>".$result['time']."</td>"
            ."<td><a href=\"mail.php?action=new&userid=".$result['idUserFrom']."\">".$result['login']."</a></td>"
            ."<td><a style=\"display:block;\" href=\"message.php?messageid=".$result['id']."\">".$result['theme']."</a></td>"
            ."<td><a class=\"remove-button\" href=\"mail.php?action=income&remove=t&messageid=".$result['id']."\"><big><i class=\"icon-remove\"></i></big></a></td></tr>";
    }
    ?></tbody>
</table>
<script>
