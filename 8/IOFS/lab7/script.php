...
$textLength = strlen($text) * PERCENTAGE / 100;
foreach ($sentences as $key => $value) {
	$sentences[$key] = Array(
		'sentence' => $value, 
		'lower' => mb_strtolower($value, mb_detect_encoding($value)), 
		'isUsed' => false
		);
}
...
foreach ($keywords as $word) {
	foreach ($sentences as $key => $sentence) {
		if (strpos($sentence['lower'], $word) > 0) {
			$sentences[$key]['isUsed'] = true;
		}
	}
	$output = '';
