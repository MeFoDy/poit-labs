...
namespace CorelDrawKiller
{
	...
	class Trapecy : Figure
	{
		...
		public override void Draw()
		{
			...
			thirdPoint.X = startPoint.X + (secondPoint.X - firstPoint.X) / 2 + (secondPoint.X - firstPoint.X) / 4;
			thirdPoint.Y = finishPoint.Y;
			fourthPoint.X = startPoint.X + (secondPoint.X - firstPoint.X) / 2 - (secondPoint.X - firstPoint.X) / 4;
			fourthPoint.Y = finishPoint.Y;
			Pen figurePen = new Pen(color, density);
			CDKForm.graphicsField.DrawPolygon(figurePen, new Point[] { firstPoint, secondPoint, thirdPoint, fourthPoint });
			DrawBorder();
		}
	}
}
